            ...
            <div className="relative z-10 flex flex-col pt-6 pb-4 pl-4 md:pl-5 pr-10 md:pr-16 w-full h-full overflow-y-auto overflow-x-hidden">
                ...
                <div className="flex flex-col shrink-0">
                    <Link href="/" className="flex flex-col gap-2">
                        <div className="relative w-[60%] aspect-[2.5/1]">
                            <Image src="/assets/dwarslogo_website.png" alt="Logo" fill className="object-contain object-left" priority />
                        </div>
                        <div className="relative w-[45%] aspect-[1.6/1] opacity-90 hover:opacity-100 hover:scale-[1.03] hover:-rotate-1 transition-all">
                ...
                <div className="mt-4 w-full flex-1 flex flex-col justify-center gap-3">
                    {/* Search */}
                    <div className="relative flex items-center w-[85%]">
                        <input
                            ...
                            placeholder="Zoeken?!"
                            className="bg-transparent placeholder-white/70 text-white font-heading tracking-widest text-sm md:text-[15px] border-white border-[1px] rounded-full py-0.5 px-3 w-full focus:outline-none focus:border-dwars-pink"
                        />
                        <div className="absolute right-2.5 w-3 h-3 md:w-3.5 md:h-3.5 flex items-center justify-center pointer-events-none">
                            <Image src="/assets/icons/icons_zoeken.png" width={12} height={12} alt="Search" className="brightness-0 invert object-contain" />
                        </div>
                    </div>

                    {/* Navigation Items */}
                    <nav className="text-white flex flex-col gap-2 font-brother font-bold tracking-wide uppercase">
                        {navItems.map((item) => (
                            <Link key={item.label} href={item.href} className="flex items-center gap-2 group cursor-pointer hover:text-dwars-pink transition-colors">
                                <div className="w-3 md:w-3.5 h-3 md:h-3.5 relative shrink-0">
                                    <Image src={item.icon} alt={item.label} fill className="object-contain" />
                                </div>
                                <p className="text-[14px] md:text-[15px] group-hover:drop-shadow-md">{item.label}</p>
                            </Link>
                        ))}
                    </nav>
                </div>
                ...
                {/* Dark Mode Toggle */}
                <div className="mt-3 flex w-[75%] mb-3">
                    <ThemeToggle />
                </div>

                {/* Tags at bottom */}
                <div className="mt-auto pt-4 w-full flex flex-col gap-1.5 shrink-0 overflow-hidden pr-2">
                    <div className="flex flex-row items-center justify-start h-6 md:h-[34px] xl:h-10 w-full gap-1.5 md:gap-2">
